...
def _utf8_subprocess_options(env=None):
    import os
    child_env = dict(env or os.environ)
    child_env["PYTHONIOENCODING"] = "utf-8"
    return {"encoding": "utf-8", "errors": "replace", "env": child_env}
...
def cmd_stability_check(args=None):
    ...
    if full_mode:
        ...
        try:
            ...
            result = _sp.run(
                ...
                capture_output=True, **_utf8_subprocess_options(env),
                stdin=_sp.DEVNULL
            )
    ...
    if cp_script.exists():
        try:
            cp = _sp.run([sys.executable, str(cp_script)], cwd=str(PROJECT_ROOT),
                         timeout=30, capture_output=True, **_utf8_subprocess_options())
    ...
    if full_mode:
        ...
        try:
            ...
            if db.exists():
                ...
                smoke_parts.append(f"DB:{_ascii_mark(has_chapters and has_fts)}")
                if not has_chapters or not has_fts:
                    smoke_ok = False
            else:
                smoke_parts.append("DB:FAIL")
                smoke_ok = False

            # b) config 可解析
            cfg_path = PROJECT_ROOT / "config.json"
            smoke_parts.append(f"CFG:{_ascii_mark(cfg_path.exists())}")

            # c) workspace 初始化
            ws = PROJECT_ROOT / "workspace"
            has_reg = (ws / "registry.json").exists()
            smoke_parts.append(f"WS:{_ascii_mark(has_reg)}")
            if not has_reg:
                smoke_ok = False
    ...
    if full_mode:
        ...
        try:
            demo = _sp.run(
                ...
                capture_output=True, **_utf8_subprocess_options(),
                stdin=_sp.DEVNULL
            )
    ...
    for name, ok, detail in checks:
        print(f"  [{_ascii_mark(ok)}] {name}: {detail}")
