"""mental_state_guard.py — 角色精神状态审核 v0.7.1

4 条规则检查角色精神状态描写是否过度/一致/合理/连续追踪。

注册名: mental_state_guard
函数:   run_mental_state_check
"""
...
def _load_genre_preset(genre: str = "default") -> dict:
    ...
    try:
        ...
        ms = data.get("mental_state", {})
        preset = ms.get(genre, ms.get("default", {}))
        return preset
    except Exception:
        return {}
...
def run_mental_state_check(
    ...
    Args:
        ...
        character_name: 指定角色（None = 检查所有有 mental_state 的角色）
        genre: 题材（用于弹性阈值）
    ...
    if density > block_threshold:
        issues.append({
            "code": "MENTAL_OVERPLAY_BLOCK",
            "severity": "FAIL",
            "message": f"精神病理词汇密度 {density:.1f}/千字，超过拦截阈值 {block_threshold}",
            "suggestion": f"减少「{'/'.join(OVERPLAY_KEYWORDS[:5])}」等词汇的使用频率",
            "confidence": 0.85,
        })
    elif density > warn_threshold:
        issues.append({
            "code": "MENTAL_OVERPLAY_WARN",
            "severity": "WARN",
            "message": f"精神病理词汇密度 {density:.1f}/千字，超过警告阈值 {warn_threshold}",
            "suggestion": "检查是否有过度渲染精神状态的段落",
            "confidence": 0.80,
        })

    # ── 角色精神状态数据读取 ──
    if not project_root:
        from src.cli.shared import PROJECT_ROOT as _root
        project_root = _root
    ...
    # 获取所有角色的精神状态数据
    mental_cards = list_mental_states(project_root)
    if character_name:
        mental_cards = [c for c in mental_cards if c.get("name") == character_name]
    ...
    for mc in mental_cards:
        name = mc.get("name", "")
        ms = mc.get("mental_state", {})
        if not ms:
            continue
        ...
        for cat, data in ms.items():
            ...
            # Rule 3: 偏离检测 — 情绪极端词密度 vs severity（仅分析角色附近文本）
            if severity > 0:
