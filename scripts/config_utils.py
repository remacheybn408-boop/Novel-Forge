#!/usr/bin/env python3
"""Shared config helpers for Novel Pipeline.

Keeps old top-level config keys and the newer nested config shape compatible.
"""
...
def normalize_config(raw: dict | None) -> dict:
    """Normalize config.json/config.example.json into the legacy runtime shape.

    Runtime scripts still read keys such as ``db_path`` and ``word_count`` at the
    top level. Newer release configs group them under ``paths`` / ``novel`` /
    ``gates``. This function makes both forms work.
    """
    ...
    gate_wc = gates.get("word_count") if isinstance(gates.get("word_count"), dict) else {}
    ...
    # Normalize word_count. Keep existing detailed rules if present, but allow
    # gates.word_count to override the normal chapter rule used by wc/post.
    ...
    min_normal = gate_wc.get("min_normal_chapter", normal.get("min", 1300))
    target_normal = gate_wc.get("target_normal_chapter", normal.get("best_min", 1900))
    long_normal = gate_wc.get("target_long_chapter", normal.get("max", 3300))
    ...
    default_wc = {
        ...
        "authorized_short": {"min": gate_wc.get("min_short_chapter", 300), "best_min": 500, "best_max": 900, "max": 1000},
        "fragment": {"min": gate_wc.get("min_short_chapter", 300), "best_min": 500, "best_max": 900, "max": 1000},
    }
    merged_wc = {**default_wc, **wc}
    merged_wc["normal"] = {**default_wc["normal"], **normal, "min": min_normal, "best_min": target_normal, "max": long_normal}
