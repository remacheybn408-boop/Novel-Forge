...
def render_status_summary(status: dict) -> str:
    ...
    if status.get("ok", False) and status.get("novel_title"):
        ...
        lines.append(f"  💡 推荐：先运行 python novel.py init")
    return "\n".join(lines)
...
def render_user_guide() -> str:
    ...
    lines.append("  " + "━" * 60)
    lines.append("  使用 python novel.py menu 进入交互式菜单")
    lines.append("  " + "━" * 60)
    lines.append("")
...
if __name__ == "__main__":
    ...
    if len(sys.argv) > 1 and sys.argv[1] == "--hermes":
        print(render_main_menu(load_project_status(), style="hermes"))
    elif len(sys.argv) > 1 and sys.argv[1] == "--guide":
        print(render_user_guide())
    elif len(sys.argv) > 1 and sys.argv[1] == "--status":
        print(json.dumps(load_project_status(), ensure_ascii=False, indent=2))
        ...
        print(render_main_menu(load_project_status(), style="cli"))
