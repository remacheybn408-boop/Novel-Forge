## v0.7.5 — 写作体验收口与稳定性 (2026-06-07)

### Added
- Web 写作页自动保存、保存状态、覆盖前轻量备份和长文本 debounce
- 润色、扩写、缩写、重写的本地 AI adapter 入口与结果预览
- 相关角色、地点、设定的轻量规则扫描侧栏
- 简单模式 / 高级模式与统一 Novel Agent 审稿入口

### Changed
- Web UI、REST API 和 API 依赖正式纳入版本库
- Dashboard 改为继续写作导向，复杂 Agent 默认隐藏
- 核心新接口统一返回 `ok/data/output/error`，兼容旧 `success` 字段

### Fixed
- active slot 的章节读取与保存路径保持一致
- 保存覆盖前使用原子替换并限制每章最多 10 份备份
- slug、章节目录和上传文件名增加路径安全校验

### Known Issues
- AI 快捷操作需要本地 AI adapter；未配置时返回明确的不可用提示
- Vite 生产构建仍提示主包体积超过默认 500 kB 阈值
